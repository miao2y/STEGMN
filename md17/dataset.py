...
class MD17Dataset:
    ...
    def __init__(
        self, partition, max_samples, delta_frame, num_past, data_dir, molecule_type
    ):
        ...
        try:
            ...
            train_idx = np.random.choice(
                np.arange(_x.shape[0]), size=int(train_par * _x.shape[0]), replace=False
            )
            ...
            val_idx = np.random.choice(
                rest, size=int(val_par * _x.shape[0]), replace=False
            )
            for _ in val_idx:
                flag[_] = 1
            rest = [_ for _ in range(_x.shape[0]) if not flag[_]]
            test_idx = np.random.choice(
                rest, size=int(test_par * _x.shape[0]), replace=False
            )
        ...
        # 接下来计算每一个点的速度，也就是这帧的位置减下一个帧
        v = x[1:] - x[:-1]
        v = v[:, z > 1, ...]
        v_0 = np.stack([v[[k + i * delta_frame for i in range(num_past)]] for k in st])
            ...
            for j in range(n_node):
                if i != j:
                    ...
                    if self.atom_edge[i][j]:
                        rows.append(i)
                        cols.append(j)
                        edge_attr.append([mole_idx[i], mole_idx[j], 1.0])
                        assert not self.atom_edge2[i][j]
                    if self.atom_edge2[i][j]:
                        rows.append(i)
                        cols.append(j)
                        edge_attr.append([mole_idx[i], mole_idx[j], 2.0])
