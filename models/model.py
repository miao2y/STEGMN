...
class SpatialBlock(nn.Module):
    ...
    def reset_parameters(self):
        stdv1 = 1.0 / math.sqrt(self.Theta1.shape[1])
...
class STAG(nn.Module):
    ...
    def __init__(
        ...
        self.spatial = SpatialBlock(
            in_channels=num_features, out_channels=8, num_nodes=num_nodes
        )
        ...
        self.encoder = Encoder(
            ...
            d_inner=12,
            dropout=0.1,
            n_position=num_timesteps_input,
            scale_emb=False,
        )

        self.theta = nn.Parameter(
            torch.FloatTensor(num_timesteps_input * 8, num_timesteps_output * out_dim)
        )
    ...
    def forward(self, A, X):
        ...
        out2 = self.encoder(
            src_seq=out1.reshape(-1, out1.shape[2], out1.shape[3]),
            src_mask=None,
            return_attns=False,
        )[0]
...
class ESTAG(nn.Module):
    ...
    def forward(self, h, x, edges, edge_attr, node_vel, cfg):
        ...
        # node_sh = self.sh_init(h, x, edges, edge_attr)
        # edge_attr = edge_attr.unsqueeze(1).repeat(1,h.shape[1],1)
        ...
        for i in range(self.n_layers):
            ...
            if self.eat:
                ...
                x = rearrange(
                    x, " (bs_n seq) x_dim -> bs_n seq x_dim", seq=self.num_past
                )
                h = rearrange(
                    h, " (bs_n seq) x_dim -> bs_n seq x_dim", seq=self.num_past
                )
                h, x = self._modules["egcl_at_%d" % (i * 2 + 2)](h, x)
                h = rearrange(
                    h, "bs_n seq h_dim -> (bs_n seq) h_dim", seq=self.num_past
                )
                x = rearrange(
                    x, "bs_n seq x_dim -> (bs_n seq) x_dim", seq=self.num_past
                )
...
class TimeBlock(nn.Module):
    ...
    def forward(self, X):
        ...
        X = X.permute(
            0, 3, 1, 2
        )  # 100 13 10 6 -> 100 6 13 10  #100 13 8 16 #100 13 6 64 # 100 13 4 16
...
class STGCN(nn.Module):
    ...
    def __init__(
        ...
        self.block1 = STGCNBlock(
            in_channels=32, out_channels=64, spatial_channels=32, num_nodes=num_nodes
        )
        self.block2 = STGCNBlock(
            in_channels=64, out_channels=64, spatial_channels=32, num_nodes=num_nodes
        )
        self.last_temporal = TimeBlock(in_channels=64, out_channels=64)

        #### 1 * 5 = 1 * (2 * 2 + 1)   (1 is kernel_size-1,    block1 -2  |  block2 -2  | last_temporal, -1)
        self.fully = nn.Linear(
            (num_timesteps_input - 1 * 5) * 64, num_timesteps_output * out_dim
        )
