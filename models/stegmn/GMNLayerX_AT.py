...
class GMNLayerX_AT(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(),
        ...
        """
        super(GMNLayerX_AT, self).__init__()
    ...
    def update(self, x, v, f, h,attention_weights, node_index, type='Isolated'):
        ...
        """
        # 检查是否为多时间步输入
        is_multi_timestep = len(x.shape) == 4  # [T, N, 4, 3]
        ...
        if is_multi_timestep:
            ...
            # 单时间步输入，直接调用单时间步版本
            return self.update_single_timestep(x, v, f, h, node_index, type)
        ...
        if type == 'Isolated':
            ...
            if self.learnable:
                ...
                if attention_weights is not None:
                    ...
                    beta_attended_t = beta_attended_t.view(N_total, 4, 3)  # [2130, 4, 3]
    ...
    def forward(self, h, edge_index, x, vec, cfg, edge_attr=None, node_attr=None):
        ...
        for type in cfg:
            x, vec = self.update_X(x, vec, f, h,attention_weights, node_index=cfg[type], type=type)
        h_out, agg = self.node_model_X(h, edge_index, edge_feat, node_attr, others=h)
        
        
        return h_out, x, vec, edge_attr, score
