...
class GMNLayerX_Pooling(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(),
        ...
        """
        super(GMNLayerX_Pooling, self).__init__()
    ...
    def update(self, x, v, f, h,attention_weights, node_index, type='Isolated'):
        ...
        """
        # 检查是否为多时间步输入
        x_t_out, v_t_out = self.update_single_timestep(x.shape[0]-1,x, v, f, h,attention_weights, node_index, type)
    ...
    def isolate_pooling(self, x, v, f, h, node_index):
        ...
        x_diff_weighted = torch.einsum("ij,jkts->ikts", self.theta, x_diff)  # [1, K, 4, 3]
    ...
    def update_single_timestep(self, t, x_T, v_T, f_T, h_T,attention_weights, node_index, type='Isolated'):
        ...
        if type == 'Isolated':
            ...
            if self.learnable:
                ...
                beta_expanded = torch.zeros(N_total, 4, 3, device=_beta.device)  # [2130, 4, 3]
                ...
                # 2. 收集所有时间步的角加速度
                beta_all_timesteps = torch.zeros(N_total, x_T.shape[0], 4, 3, device=_beta.device)  # [2130, T, 4, 3]
                ...
                beta_reshaped = beta_all_timesteps.permute(1,0,2,3) 
                
                # 使用注意力权重计算所有时间步的beta
                # attention_weights: [N, T, T]
                # 意思是每个残基都对应 10 个时间步的注意力权重
                _beta=torch.einsum("ij,jkts->ikts", self.theta, beta_reshaped).squeeze(0)+beta_reshaped[-1]
                _beta_attended_1 = _beta[id1]  # [970, 4, 3]
                _beta_attended_2 = _beta[id2]  # [970, 4, 3]
                    
                # 使用两个节点注意力加权的角加速度的平均值
                _beta = (_beta_attended_1 + _beta_attended_2) / 2
                # compute c metrics
    ...
    def stick_pooling(self, x, v, f, h,attention_weights, node_index, type='Isolated'):
    ...
    def forward(self, h, edge_index, x, vec, cfg, edge_attr=None, node_attr=None):
        ...
        # 只取T-1时刻的权重，对V做加权和，作为T-1时刻的特征
        attention_weights[:, T-1, :]  # [N, T]
        # v_T_weighted = torch.einsum('nt,ntd->nd', attn_T_minus_1, v)  # [N, hidden_nf]

        # # 构造新的h_attended，只有T-1时刻为加权和，其余为原始h
        # h_attended = h.clone()  # [T, N, hidden_nf]
        # h_attended[T-1] = v_T_weighted
        ...
        # 执行pooling操作
        x_isolated_pooled, vec_isolated_pooled = self.isolate_pooling(x=x, v=vec, f=f, h=h, node_index=cfg["Isolated"])
        x_stick_pooled, vec_stick_pooled = self.stick_pooling(x, vec, f, h, attention_weights, node_index=cfg["Stick"], type="Stick")
        
        # 合并pooling结果
        x_final, vec_final = self.merge_pooled_results(
            x_isolated_pooled, vec_isolated_pooled,
            x_stick_pooled, vec_stick_pooled,
            cfg["Isolated"], cfg["Stick"],
            N, x.device
        )
