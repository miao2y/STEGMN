...
class GMNLayer_Pooling(nn.Module):
    def __init__(self, input_nf, output_nf, hidden_nf, edges_in_d=0, nodes_att_dim=0, act_fn=nn.ReLU(),
        ...
        """
        super(GMNLayer_Pooling, self).__init__()
    ...
    def reset_parameters(self):
        # time_weights init
        self.time_weights.data.uniform_(-1, 1)
        # O init
        self.time_weights.data *= 0
        # theta init
    ...
    def forward(self, h, edge_index, x, vec, cfg, edge_attr=None, node_attr=None):
        # x, vec shape: [10, 1300, 3]
        # cfg: 包含 'Isolated' 和 'Stick' 配置的字典
        # 获取最后一个时刻（t=10）的坐标
        x_last = x[-1]  # [1300, 3] - 最后一个时刻的所有节点坐标
        vec_last = vec[-1]  # [1300, 3] - 最后一个时刻的所有节点速度
        ...
        if 'Stick' in cfg:
            ...
            if stick_config.shape[0] > 0:
                ...
                # 获取所有节点的位置和速度 [num_sticks, 10, 3]
                x1_all = x[:, node1_indices, :]  # [10, num_sticks, 3]
                x2_all = x[:, node2_indices, :]  # [10, num_sticks, 3]
                v1_all = vec[:, node1_indices, :]  # [10, num_sticks, 3]
                v2_all = vec[:, node2_indices, :]  # [10, num_sticks, 3]
                
                # 计算所有stick的中点 [10, num_sticks, 3]
                x_center_all = (x1_all + x2_all) / 2
                v_center_all = (v1_all + v2_all) / 2
                
                # 计算相对位置和相对速度 [10, num_sticks, 3]
                r1_all = x1_all - x_center_all
                r2_all = x2_all - x_center_all
                v_rel1_all = v1_all - v_center_all
                v_rel2_all = v2_all - v_center_all
                
                # 计算角速度: ω = (r × v) / |r|² [10, num_sticks, 3]
                r1_norm = torch.norm(r1_all, dim=-1, keepdim=True).clamp_min(1e-5)
                r2_norm = torch.norm(r2_all, dim=-1, keepdim=True).clamp_min(1e-5)
                ...
                # 初始化角速度存储张量 [num_unique_nodes, 10, 3]
                angular_velocities = torch.zeros(len(all_nodes), 10, 3, device=x.device)
                ...
                angular_velocities.index_add_(0, node1_positions, omega1_all.permute(1, 0, 2))  # [num_sticks, 10, 3] -> [num_unique_nodes, 10, 3]
                angular_velocities.index_add_(0, node2_positions, omega2_all.permute(1, 0, 2))
                ...
                # 平均角速度
                node_counts = node_counts.unsqueeze(-1).unsqueeze(-1)  # [num_unique_nodes, 1, 1]
                ...
                angular_velocities_summed = torch.einsum('t,ntd->nd', normalized_weights, angular_velocities)  # [num_unique_nodes, 3]
                
                # 向量化计算旋转矩阵
                def compute_rotation_matrices_vectorized(omegas, dt=1.0):
                    # omegas: [num_nodes, 3]
                    omega_norms = torch.norm(omegas, dim=-1)  # [num_nodes]
                    
                    # 创建单位矩阵 [num_nodes, 3, 3]
                    batch_size = omegas.shape[0]
                    I = torch.eye(3, device=omegas.device).unsqueeze(0).expand(batch_size, -1, -1)
                    
                    # 处理角速度很小的节点
                    small_omega_mask = omega_norms < 1e-6
                    
                    # 归一化角速度方向
                    omega_unit = omegas / omega_norms.unsqueeze(-1).clamp_min(1e-6)  # [num_nodes, 3]
                    
                    # 旋转角度
                    theta = omega_norms * dt  # [num_nodes]
                    
                    # 使用Rodrigues公式计算旋转矩阵
                    cos_theta = torch.cos(theta)  # [num_nodes]
                    sin_theta = torch.sin(theta)  # [num_nodes]
                    
                    # 创建叉积矩阵 [num_nodes, 3, 3]
                    omega_cross = torch.zeros(batch_size, 3, 3, device=omegas.device)
                    ...
                    # 计算K²
                    K_squared = torch.bmm(omega_cross, omega_cross)  # [num_nodes, 3, 3]
                    
                    # 旋转矩阵 R = I + sin(θ)K + (1-cos(θ))K²
                    sin_theta_expanded = sin_theta.unsqueeze(-1).unsqueeze(-1)  # [num_nodes, 1, 1]
                    cos_theta_expanded = cos_theta.unsqueeze(-1).unsqueeze(-1)  # [num_nodes, 1, 1]
                ...
                # 批量计算旋转矩阵 [num_unique_nodes, 3, 3]
                rotation_matrices = compute_rotation_matrices_vectorized(angular_velocities_summed, dt=1.0)
                
                # 计算所有stick的中心点 [num_sticks, 3]
                x1_last_all = x_last[node1_indices]  # [num_sticks, 3]
                x2_last_all = x_last[node2_indices]  # [num_sticks, 3]
                x_center_last_all = (x1_last_all + x2_last_all) / 2  # [num_sticks, 3]
                
                # 计算相对位置向量 [num_sticks, 3]
                r1_last_all = x1_last_all - x_center_last_all
                r2_last_all = x2_last_all - x_center_last_all
                
                # 向量化计算新位置
                # 获取对应的旋转矩阵 [num_sticks, 3, 3]
                R1_all = rotation_matrices[node1_positions]  # [num_sticks, 3, 3]
                R2_all = rotation_matrices[node2_positions]  # [num_sticks, 3, 3]
                
                # 批量计算旋转后的相对位置 [num_sticks, 3]
                r1_rotated = torch.bmm(R1_all, r1_last_all.unsqueeze(-1)).squeeze(-1)  # [num_sticks, 3]
                r2_rotated = torch.bmm(R2_all, r2_last_all.unsqueeze(-1)).squeeze(-1)  # [num_sticks, 3]
                ...
                all_next_positions = torch.cat([x1_next_all, x2_next_all])  # [2*num_sticks, 3]
